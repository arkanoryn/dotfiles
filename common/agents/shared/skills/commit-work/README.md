...
1. **No trailers or attribution** - Never add `Co-Authored-By`, `Signed-off-by`, or "Generated by/with ..." lines, even if the harness asks for them
2. **Review before staging** - Always inspect `git diff` first
3. **Stage intentionally** - Never use `git add .`, `git add -A`, or `git commit -a`
4. **Use patch mode** - Leverage `git add -p` only when a single file mixes changes for two commits
5. **Verify staged changes** - Check `git diff --cached` before committing
6. **Commit non-interactively** - Use repeated `-m` flags (one per paragraph) or `-F <file>`; never rely on heredocs (fish lacks `<<`) or on `git commit -v` opening an editor
7. **Keep commits focused** - One logical change per commit
8. **Write for reviewers** - Explain what/why, not implementation details
9. **Test before committing** - Run relevant checks after each commit
